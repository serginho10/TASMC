...
public class PanAndZoomListener implements OnTouchListener {
    ...
    public PanAndZoomListener(FrameLayout containter, View view, int anchor, Context context) {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                mode = DRAG;
                //para boton
                int bitmapXPosition = 500;
                int bitmapWidth = 35;
                int bitmapYPosition = 1250;
                int bitmapHeight = 35;
                if( x > bitmapXPosition && x < bitmapXPosition + bitmapWidth && y > bitmapYPosition && y < bitmapYPosition + bitmapHeight ){
                    Toast.makeText(context,"Prueba de boton", Toast.LENGTH_LONG).show();
                    System.out.println("xcv,jbhhb,nmblb,bnmbjkh,b");
                }
    ...
    class PanZoomCalculator {
        ...
        public void doZoom(float scale, PointF zoomCenter) {
            ...
            currentZoom = Math.min(100f, currentZoom);
